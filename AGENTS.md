...
- `@react-pdf/renderer` must be **dynamically imported** (`await import('@react-pdf/renderer')`) when generating blobs (see `Previewer.tsx`). Fonts are registered in `src/components/pdf/styles.ts` via `?url` asset imports. `main.tsx` does NOT import the pdf tree — it is only reachable through the lazy `Previewer`, so the renderer stays out of the initial bundle. `doctor.config.json` overrides `prefer-dynamic-import` for `src/components/pdf/**` for the same reason.
